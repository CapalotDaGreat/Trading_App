...
import { pickDrillForConcept, pickReplayForConcept, targetComplexityFor } from './adaptive-difficulty.service';
...
function candidatesFromCompetency(
  ...
  for (const row of rows) {
    ...
    const complexity = targetComplexityFor(snapshot.byConcept[row.conceptId]);
    ...
    const picked = pickHrefForStep({
      ...
      recent,
      rem: row.remediation,
      promptHref: row.nextDemonstration?.href,
      snapshot,
    });
    ...
    const why = whyFor({
      ...
      conceal: Boolean(conceal && (priority === 'redemonstration' || loopStep === 'redemonstrate')),
      deferCount,
    ...
    out.push({
      ...
      priority,
      loopStep,
      kind: priority === 'remediation' ? 'remediation' : priority === 'redemonstration' ? 'redemonstration' : picked.kind,
      whyToday: conceal ? deliberateWhy(scaffolding, why) : why,
      evidence: row.explanations.slice(0, 3),
      href: picked.href,
      complexity: beginner ? 'foundations' : complexity,
      concealConcept: Boolean(conceal && (priority === 'redemonstration' || loopStep === 'redemonstrate' || scaffolding.concealConcept)),
      deferCount,
    });
  }
...
function eventPrepItem(snapshot: LearningEvidenceSnapshot, masteryHasEvent: boolean): TrainingQueueItem | null {
  const canEvent =
    snapshot.eventPlan &&
    snapshot.experience &&
...
function toItem(candidate: Candidate): TrainingQueueItem {
...
function headlineFor(state: TrainingEmptyState, lead: TrainingQueueItem | undefined): string {
...
/**
 * Adaptive Today’s Training. Priority is explainable and process-based.
...
export function composeTodaysTraining(
  ...
  if (!items.some((item) => item.kind === 'simulation_challenge')) {
    items.push(toItem(simulationCandidate(snapshot)));
  }
  if (!items.some((item) => item.kind === 'journal_review')) {
    items.push(toItem(journalCandidate(snapshot)));
  }
  ...
  const eventItem = eventPrepItem(snapshot, masteryHasEvent);
  if (eventItem && !beginner) {
    items.push(eventItem);
  }

  const visibleItems = items.filter((item) => visible(item, dispositions, now));
  let queued = visibleItems.slice(0, 8);

  if (
    grinding.grinding &&
    queued[0] &&
    queued[0].priority !== 'remediation' &&
    queued[0].priority !== 'redemonstration' &&
    (queued[0].priority === 'curriculum' || queued[0].kind === 'continue_lesson')
  ) {
    const mixed =
      queued.find((item) => item.kind === 'simulation_challenge' || item.concealConcept) ??
      toItem({
        id: 'anti-grind-mixed',
        ...
        loopStep: 'apply',
        kind: 'simulation_challenge',
        whyToday: grinding.reason ?? 'Mixed independent practice is the next step.',
        evidence: ['Easy-session volume is not mastery.'],
    ...
    queued = [
      {
        ...mixed,
        whyToday: `${grinding.reason} ${mixed.whyToday ?? mixed.reason}`.trim(),
        reason: grinding.reason ?? mixed.reason,
      },
      ...queued.filter((item) => item.id !== mixed.id),
    ];
  }

  const leadItem = queued[0];
  const interleaved = leadItem ? [leadItem, ...interleaveByFamily(queued.slice(1))] : queued;
  const filtered = interleaved.slice(0, 6);
  ...
  const emptyState = emptyStateOf(snapshot, competency, lead);

  const continueId = snapshot.openedLessonIds[0] ?? snapshot.nextLessonId;
  const lessonChain = continueId
    ? nextAfterLesson(
        continueId,
        targetComplexityFor(snapshot.byConcept[primaryConceptForLesson(continueId)?.id ?? '']),
      )
    : null;

  return {
    headline: headlineFor(emptyState, lead),
    coachLine:
      'Why this is today’s training is explained on each item. Skip or defer anything — you are never locked out. Process evidence decides the queue; simulated P/L does not.',
