...
import { selectNextDemonstration } from './context.service';
...
import { buildRemediationPlan } from './context.service';
...
function isDemonstration(record: CompetencyEvidenceRecord): boolean {
  if (isExposureOnlySource(record.sourceType)) return false;
...
function gradedRecords(records: CompetencyEvidenceRecord[]): CompetencyEvidenceRecord[] {
  return records
    .filter((item) => !isExposureOnlySource(item.sourceType) && outcomeValue(item.result) != null)
...
function strengthOf(records: CompetencyEvidenceRecord[], now: number): number | null {
  ...
  for (const item of records) {
    if (isExposureOnlySource(item.sourceType)) continue;
    const raw = outcomeValue(item.result);
    if (raw == null) continue;
    const independent = item.independent && !item.hintsUsed;
...
function recordFitsRequirement(
  ...
  if (!record.independent || record.hintsUsed) return false;
  if (record.result !== 'pass' && record.result !== 'partial') return false;
...
export function missingRecipeRoles(
  ...
  const remaining = records
    ...
    .slice()
    .sort((a, b) => a.occurredAt - b.occurredAt);
  const pool = remaining.filter((item) => item.independent && !item.hintsUsed && item.result !== 'fail');
  const unused = [...pool];
...
export function scoreCompetencyMastery(
  ...
  const independentDemos = demonstrations.filter((item) => item.independent && !item.hintsUsed);
  ...
  const needsRemediation = !recoveredAfterRemediation && hadRepeatedProcessMisses(graded);
  ...
  if (mine.length === 0) {
    ...
    explanations.push(
      previouslyDemonstrated
        ? 'Earlier demonstrations still count. Recent performance shows a recurring process miss — this needs more practice.'
        : 'Recent performance shows a recurring process miss. This needs more practice.',
    );
  } else if (stale && previouslyDemonstrated) {
    state = 'due_for_redemonstration';
    explanations.push('Due for review. Re-demonstrate the process in a different scenario — recency matters.');
  } else if (demonstratedEligible) {
    state = 'demonstrated';
    explanations.push(
      'Demonstrated inside TradeAcademy’s training environment across more than one kind of evidence. Process counts; simulated P/L does not.',
    );
  } else if (demonstrations.length > 0 || graded.length > 0) {
    state = 'practiced';
    if (recoveredAfterRemediation && !freshAfterRemediation) {
      explanations.push('Remediation was completed. Evidence history is kept. A fresh independent demonstration is still needed.');
    }
    if (missingRoles.length) {
      explanations.push(`Still developing: ${missingRoles.join(', ')} evidence is missing before this can be demonstrated.`);
    } else if (!freshAfterRemediation) {
      explanations.push('A successful remediation does not skip back to demonstrated. Show the process independently once more.');
    }
  ...
  const remediation = state === 'needs_remediation' ? buildRemediationPlan(canonicalId) : null;
  if (remediation) {
    remediation.diagnosis = diagnosisFromFlags(mine, remediation.diagnosis);
  }

  const nextDemonstration =
    state === 'due_for_redemonstration' || state === 'needs_remediation' || (state === 'practiced' && missingRoles.length > 0)
  ...
  const userLabel = userLabelFor(state, independentDemos.length, strength);
