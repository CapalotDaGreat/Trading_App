...
export function usePortfolio() {
  ...
  const symbols = useMemo(() => storedHoldings.map((h) => h.symbol), [storedHoldings]);
  ...
  const updateMutation = useMutation({
    mutationFn: ({ holdingId, updates }: { holdingId: string; updates: UpdateHoldingInput }) =>
  ...
  return {
    ...
    holdingPnLs,
    performance:
      performanceQuery.data && performanceQuery.data.points.length > 1
