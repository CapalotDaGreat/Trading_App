...
const FOREGROUND_ONLY_SUMMARY =
  'Alerts are checked about every 45 seconds while TradeAcademy is open. Background delivery is not available in this build — keep the app open, or use an EAS Dev Client / production install for OS-scheduled background checks.';

const BACKGROUND_AVAILABLE_SUMMARY =
  'While TradeAcademy is open, alerts check about every 45 seconds. When the app is backgrounded or closed, the OS may wake TradeAcademy periodically to re-check (often 15+ minutes later — not instant). Battery savers and low power can delay delivery.';
