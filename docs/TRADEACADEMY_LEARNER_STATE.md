...
| Evidence ledger | `tradeacademy-competency-evidence-v1` | No | `users/{uid}/learnerEvidence/{eventKey}` |
| Planner / queue | `tradeacademy-learning-queue-v1` | No | Inside `users/{uid}/learnerState/progress` |
| Academy progress | `tradeacademy-academy-progress` | No | Progress doc (`lessons`, quiz stats) |
| Practice history | `tradeacademy-practice-progress-v1` | No | Progress doc |
| Replay progress | `tradeacademy-replay-tv-v2` (uid map) | No | Completed episode IDs + process scores — **not** draft reasoning |
| Learner behavior | `tradeacademy-learner-behavior-v1` | No | Compact events / self-confidence numbers |
| Learner profile (coach) | `tradeacademy:coach-profile:v1:{uid}` | `users/{uid}` preferences | Unchanged |
...
| Simulation **book / equity** | `tradeacademy-simulation-v1` | Rules existed; feature was local | Process counts only in progress meta. Cash/equity are not synced |
...
1. Offline → mark `tradeacademy-learner-sync-queue-v1` pending; keep local writes.  
